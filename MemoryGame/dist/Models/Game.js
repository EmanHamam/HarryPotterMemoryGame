...
export class Game {
    constructor(characters, crestImg) {
        this.timer = null;
    ...
    handleClick(uid) {
        if (this.state.isLocked || this.state.isGameOver)
            return;
    ...
    buildDeck() {
        ...
        const deck = doubled.map((char, index) => new Card(index + 1 + Math.random(), char.id, char.image));
